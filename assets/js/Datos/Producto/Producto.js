...
function cancelarp(){
	$("#btn-guardarProducto").attr('onclick', 'guardarp()')
	$("#txtNombreProducto").val('')
	$("#txtMarca").val('')
	$("txtCostoUnitaria").val('')
	$("#txtPorcentajeG").val('')
	...
	$("#cbxTipoProducto").val('default').selectpicker('refresh')
	$("#cbxUnidadMedida").val('default').selectpicker('refresh')
}
		...
		success:function(data){
			...
			$.each(tabla, function(i, item){
				...
				$("#tbodyProducto").append(
					...
					'<td>'+
					'<button type="button" class="btn palette-Light-Blue bg" onclick="PasarParamVenProducto(\'' + item.nombreproducto + '\', \'' + item.Marca + '\', \'' + item.unidadId + '\', \'' + item.costounitario + '\', \'' + item.idTipoProducto + '\', \'' + item.ProductoId + '\')"><i class="zmdi zmdi-account"></i></button>'+
...
function listarProductoCombo(){
	$.ajax({
		url: ruta_universal + 'Producto_controllers/listarProductoCombo',
		type:'POST',
		cache:false,
		success:function(data){
...
function PasarParamVenProducto(nombreproducto, Marca, unidadId, costounitario, idTipoProducto, ProductoId){
